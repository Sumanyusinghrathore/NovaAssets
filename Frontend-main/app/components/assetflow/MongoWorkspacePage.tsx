...
import { ROLE_LABELS, sectionHref, type AssetFlowRole, type AssetFlowSection } from "@/app/lib/assetflow-roles";
import { IconArrowUpRight, IconBell, IconChartBar, IconCoins, IconFactory, IconUsers } from "@/app/components/icons/Icons";
...
export default function MongoWorkspacePage({ role, section, title, subtitle }: Props) {
  ...
  const isFounderOverview = role === "founder" && section === "dashboard";
  ...
  const heroStats = useMemo(() => {
    ...
    if (isFounderOverview) {
      return [
        { label: "Employees", value: numberFormat(users.length), hint: `${activeUsers} active accounts` },
        { label: "Access roles", value: numberFormat(roles.length), hint: `${systemRoles} protected system roles` },
        { label: "Organization groups", value: numberFormat(groups.length), hint: "Current people distribution" },
        { label: "Unread alerts", value: numberFormat(unreadCount), hint: "Items needing review" },
      ];
    }
  ...
  }, [activeUsers, demoUsers.length, groups.length, unreadCount, users.length, roles.length, section, systemRoles, isFounderOverview]);
    ...
    <section className="glass-panel soft-shadow rounded-[2rem] p-6 sm:p-8">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 rounded-full border border-[#5b3df5]/20 bg-[#5b3df5]/6 px-3 py-1 text-xs font-semibold uppercase tracking-[0.28em] text-[#5b3df5]">
            {isFounderOverview ? "Executive oversight" : "Mongo live data"}
          </div>
      ...
      {isFounderOverview && (
        <>
          <div className="mt-8 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            <SectionPanel title="Founder control flow" subtitle="Set up the organization first, then govern the operational lifecycle.">
              <div className="grid gap-3 sm:grid-cols-3">
                {[
                  {
                    step: "01",
                    title: "Set up masters",
                    detail: "Create departments and asset categories before registering assets.",
                    href: sectionHref("founder", "organization"),
                    icon: IconFactory,
                    action: "Organization",
                  },
                  {
                    step: "02",
                    title: "Manage people",
                    detail: "Keep sign-ups as Employees; promote only from the employee directory.",
                    href: sectionHref("founder", "employees"),
                    icon: IconUsers,
                  ...
                  },
                  {
                    ...
                    detail: "Use audit cycles, reports, and alerts to govern the organization.",
                    href: sectionHref("founder", "audit"),
                    icon: IconCoins,
                    action: "Audit cycles",
                  },
                ].map((item) => {
                  ...
                  return (
                    <Link
                      key={item.step}
                      href={item.href}
                      className="group rounded-[1.45rem] border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-[#5b3df5]/35 hover:shadow-md"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <span className="text-[11px] font-bold tracking-[0.28em] text-[#5b3df5]">{item.step}</span>
                        <span className="rounded-xl bg-[#eef2ff] p-2 text-[#5b3df5]"><Icon size={16} /></span>
                      </div>
                      <div className="mt-4 text-base font-black text-slate-900">{item.title}</div>
                      <p className="mt-2 text-sm leading-6 text-slate-600">{item.detail}</p>
                      <div className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#5b3df5]">
                        {item.action}<IconArrowUpRight size={14} />
                      </div>
                    </Link>
                  );
                })}
            ...
            <SectionPanel title="Attention queue" subtitle="Live signals from the current organization data.">
              <div className="grid gap-3">
                {[
                  { label: "Unread notifications", value: unreadCount, detail: "Review assignments, approvals, and overdue alerts.", icon: IconBell, href: sectionHref("founder", "notifications") },
                  { label: "Inactive employee accounts", value: inactiveUsers, detail: "Confirm whether inactive access should remain disabled.", icon: IconUsers, href: sectionHref("founder", "employees") },
                  { label: "Organization groups", value: groups.length, detail: "Review people distribution before assigning organization owners.", icon: IconFactory, href: sectionHref("founder", "departments") },
                ].map((item) => {
                  const Icon = item.icon;
                  return (
                    <Link key={item.label} href={item.href} className="flex items-center gap-3 rounded-[1.2rem] border border-slate-200 bg-white p-3.5 shadow-sm transition hover:border-[#5b3df5]/30">
                      <span className="rounded-xl bg-slate-100 p-2 text-slate-600"><Icon size={16} /></span>
                      <span className="min-w-0 flex-1">
                        <span className="block text-sm font-bold text-slate-900">{item.label}</span>
                        <span className="mt-0.5 block text-xs leading-5 text-slate-500">{item.detail}</span>
                      </span>
                      <span className="text-xl font-black text-[#5b3df5]">{numberFormat(item.value)}</span>
                    </Link>
                  );
                })}
              </div>
              <Link href={sectionHref("founder", "reports")} className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-[#5b3df5]">
                View organization reports <IconChartBar size={16} />
              </Link>
            </SectionPanel>
          </div>
        </>
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="grid gap-6">
          <SectionPanel title={isFounderOverview ? "Administration scope" : "Demo access"} subtitle={isFounderOverview ? "Documented control points for the Founder / Super Admin" : "Mongo-synced login identities"}>
            {isFounderOverview ? (
              <div className="grid gap-3">
                <div className="rounded-[1.2rem] border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="text-sm font-bold text-slate-900">Organization setup</div>
                  <p className="mt-1 text-sm leading-6 text-slate-600">Maintain departments, asset categories, and the employee directory. Promote employee accounts to Department Head or Asset Manager only from the directory.</p>
                </div>
                <div className="rounded-[1.2rem] border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="text-sm font-bold text-slate-900">Governance and oversight</div>
                  <p className="mt-1 text-sm leading-6 text-slate-600">Run audit cycles, review notifications, and use organization-wide reports and analytics to monitor asset and resource operations.</p>
                </div>
              </div>
            ) : (
              <div className="grid gap-3">
