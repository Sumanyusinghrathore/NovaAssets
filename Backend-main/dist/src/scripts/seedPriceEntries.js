...
function point(distributor, dealer, msp) {
    return { distributor, dealer, msp };
}
function prices(up, bihar, mp, haryanaRajasthan) {
    return {
        UP: up,
        Bihar: bihar,
        MP: mp,
...
function deriveModelKey(modelNo) {
...
async function main() {
    const connection = await (0, connect_1.connectDatabase)();
    ...
    for (const productDoc of products) {
        ...
        const result = await c.priceEntries.updateOne({ modelNo: productDoc.model }, { $set: doc }, { upsert: true });
