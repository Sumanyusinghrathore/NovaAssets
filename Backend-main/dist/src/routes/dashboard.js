...
function isClosedStatus(status) {
    return complaintRules_1.CLOSED_COMPLAINT_STATUSES.includes(status);
}
...
router.get("/stats", auth_1.authenticate, (0, auth_1.requireAnyPermission)("dashboard:view"), async (_req, res) => {
    ...
    const openComplaints = await c.complaints.countDocuments({ status: { $in: complaintRules_1.OPEN_COMPLAINT_STATUSES } });
