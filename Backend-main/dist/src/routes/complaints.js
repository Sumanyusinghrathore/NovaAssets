...
const ACTIVE_ENGINEER_STATUSES = [
    ...
    ...complaintRules_1.IN_PROGRESS_COMPLAINT_STATUSES,
    "Escalated to L2",
    "Escalated to L3",
    "Pending L3 Approval",
    "Spare Requested",
...
function activeQueueRank(status) {
    if (complaintRules_1.IN_PROGRESS_COMPLAINT_STATUSES.includes(status))
        return 0;
...
async function backupTicketCount(engineerId, engineerName, excludeComplaintId) {
    const c = await (0, collections_1.getCollections)();
    const filter = {
        ...engineerIdentityFilter(engineerId, engineerName),
...
router.get("/stats", auth_1.authenticate, (0, auth_1.requireAnyPermission)("complaints:consumer", "complaints:supplier"), async (_req, res) => {
    const c = await (0, collections_1.getCollections)();
    const statuses = [
        complaintRules_1.OPEN_COMPLAINT_STATUSES[0],
        "Waiting Lobby",
        "Assigned to Engineer",
        "Assigned for Onsite",
        complaintRules_1.IN_PROGRESS_COMPLAINT_STATUSES[0],
        "Escalated to L2",
        "Escalated to L3",
        "Pending L3 Approval",
        "Spare Requested",
...
router.post("/upload-inverter-picture", auth_1.authenticate, (0, auth_1.requireAnyPermission)("complaints:consumer", "complaints:supplier"), runInverterPictureUpload, async (req, res) => {
    ...
    try {
        const uploaded = await (0, cloudinary_1.uploadBufferToCloudinary)(file, "novaassets/complaint-inverter-pictures");
        if (!uploaded.url)
...
router.post("/", auth_1.authenticate, (0, auth_1.requireAnyPermission)("complaints:consumer", "complaints:supplier"), async (req, res) => {
    ...
    const complaintStatus = assignmentDecision?.status ?? complaintRules_1.OPEN_COMPLAINT_STATUSES[0];
    const complaint = {
        id: (0, id_1.generateId)(),
        type,
        productSerialNo,
        ...
        workflowHistory: [createWorkflowHistoryEvent({
                ...
                toStatus: assignment?.status ?? complaintRules_1.OPEN_COMPLAINT_STATUSES[0],
                user,
...
router.put("/:id/service", auth_1.authenticate, (0, auth_1.requireAnyPermission)("complaints:consumer", "complaints:supplier", "dispatch:manage"), async (req, res) => {
    ...
    const isStartWorkRequest = complaintRules_1.IN_PROGRESS_COMPLAINT_STATUSES.includes(req.body.status) || (("serviceStartedAt" in req.body) && req.body.serviceStartedAt);
    ...
    if ((complaintRules_1.IN_PROGRESS_COMPLAINT_STATUSES.includes(req.body.status) || ("serviceStartedAt" in req.body && req.body.serviceStartedAt)) && !existing.serviceStartedAt) {
