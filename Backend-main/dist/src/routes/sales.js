...
router.put("/:id", auth_1.authenticate, (0, auth_1.requireAnyPermission)("sales:entry", "dispatch:manage"), async (req, res) => {
    const c = await (0, collections_1.getCollections)();
    const { id } = req.params;
    const existing = await c.sales.findOne({ id });
    if (!existing)
        return (0, http_1.fail)(res, "Sale not found", 404);
    const { documentType, referenceNo, saleDate, customerId, unregisteredCustomerName, unregisteredCustomerAddress, unregisteredCustomerGst, dealerRegistered, stateRegion, serialNumber, materialName, customerName, customerPhone, customerEmail, customerAddress, customerType, } = req.body;
    ...
    if (normalizedNewSerial !== normalizedOldSerial) {
        if (normalizedOldSerial) {
            ...
            if (oldMfg) {
                await c.manufactured.updateOne({ id: oldMfg.id }, { $set: { status: "In Stock", updatedAt: new Date() }, $unset: { invoiceNo: "", customerId: "", soldDate: "" } });
            }
            await (0, serialLifecycle_1.updateSerialStatus)(c, { serialNumber: normalizedOldSerial, status: "Available" });
        }
        if (normalizedNewSerial) {
            const newMfg = await resolveManufacturedProductForSerial(c, { serialNumber: normalizedNewSerial, materialName: resolvedMaterialName, referenceNo: finalReferenceNo, saleDate, customerId, paymentStatus: existing.paymentStatus }, normalizedNewSerial);
            if (!newMfg)
                return (0, http_1.fail)(res, "Serial number not found in manufactured products");
            if (newMfg.status === "Sold")
                return (0, http_1.fail)(res, "This product is already sold");
            await c.manufactured.updateOne({ id: newMfg.id }, {
                $set: {
                    status: "Sold",
            ...
            await (0, serialLifecycle_1.updateSerialStatus)(c, { serialNumber: normalizedNewSerial, status: "Sold" });
            const product = await c.products.findOne({ id: newMfg.productId });
            if (product)
                resolvedMaterialName = product.model;
        }
    }
    if (isRegisteredCustomer && customer && (customerName || customerPhone || customerEmail !== undefined || customerAddress !== undefined || customerType)) {
        await c.customers.updateOne({ id: customer.id }, {
            $set: {
...
router.post("/upload-docket", auth_1.authenticate, (0, auth_1.requireAnyPermission)("dispatch:manage"), runDispatchDocketUpload, async (req, res) => {
    ...
    try {
        const uploaded = await (0, cloudinary_1.uploadBufferToCloudinary)(file, "novaassets/dispatch-dockets");
        if (!uploaded.url)
...
router.post("/upload-pi", auth_1.authenticate, (0, auth_1.requireAnyPermission)("sales:entry", "accounts:manage"), runPiUpload, async (req, res) => {
    ...
    try {
        const uploaded = await (0, cloudinary_1.uploadBufferToCloudinary)(file, "novaassets/pi-attachments");
        if (!uploaded.url)
            return (0, http_1.fail)(res, "Cloudinary did not return a file URL", 502);
        return (0, http_1.ok)(res, {
            fileName: file.originalname,
...
router.put("/:id/dispatch-team", auth_1.authenticate, (0, auth_1.requireAnyPermission)("dispatch:manage"), async (req, res) => {
    const c = await (0, collections_1.getCollections)();
    const { id } = req.params;
    const { serialNumber, confirmedDispatchDate, dispatchStatus, courierDocketNo, courierDocketAttachmentName, courierDocketAttachmentUrl, forwardToAccounts, vehicleNo, piItems, } = req.body;
    const sale = await c.sales.findOne({ id });
    if (!sale)
        return (0, http_1.fail)(res, "PI record not found", 404);
    const user = req.user;
    ...
    if (piItems && Array.isArray(piItems)) {
        ...
        for (const item of piItems) {
            if (item.serialNumbers && Array.isArray(item.serialNumbers)) {
                const serials = item.serialNumbers.map((s) => normalizeSerialNumber(s)).filter(Boolean);
                if (serials.length > item.quantity) {
                    return (0, http_1.fail)(res, `Cannot allocate more serial numbers than quantity (${item.quantity}) for ${item.materialName}`);
                }
        ...
        if (uniqueSerials.size !== allSerials.length) {
            return (0, http_1.fail)(res, "Duplicate serial numbers selected");
        }
            ...
            for (const serial of allSerials) {
                ...
                if (mfg) {
                    if (mfg.status === "Sold" && mfg.invoiceNo !== sale.referenceNo) {
                        return (0, http_1.fail)(res, `Product with serial ${serial} is already sold`);
                    }
